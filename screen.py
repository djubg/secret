...
def windows_grab_screen(region):
    hwin = None
    if capture_window_class_name is not None or capture_window_name is not None:
        hwin = win32gui.FindWindow(capture_window_class_name, capture_window_name)
        if not hwin:
            logger.warning(
                "Configured window not found. Falling back to desktop capture.",
                extra={
                    "capture_window_class_name": capture_window_class_name,
